...
class ExtendedMaskRCNN(MaskRCNN):
    def __init__(self, 
                 n=3,
            backbone:Optional[BackboneWithFPN]=None, 
            transform=None, 
            num_classes=2, 
            min_size=800, 
            max_size=1333,
        ...
        if not isinstance(backbone, BackboneWithFPN): # see maskrcnn_resnet50_fpn in torchvision/models/detection/mask_rcnn.py
            norm_layer = misc_nn_ops.FrozenBatchNorm2d
            trainable_backbone_layers = _validate_trainable_layers(True, trainable_backbone_layers, max_value=5, default_value=3) 
            backbone = resnet50(weights=ResNet50_Weights.IMAGENET1K_V1, progress=True, norm_layer=norm_layer)
            backbone = _resnet_fpn_extractor(backbone, trainable_backbone_layers)
            in_channels = n # greyscale
